        ...
        try:
            ...
            if values1[0] == 'Cadastrar novo voo':
                if not janelaCancelamentoVoos:
                    tableUpdate(SQLCursor=SQLCursor, SQLQuery=SQLOrderIdAviao, window=janelaPrincipal)
...
#Setup inicial
    ...
    SQLOrderIdAviao = (f"SELECT {', '.join(SQLColumns)} FROM voo "
